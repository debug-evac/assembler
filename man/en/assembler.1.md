...
By default, **assembler** translates one or more input *file*s to binary or MIF output files. The **-f** or **\-\-format** option dictates which format should be used for the output. The output is either binary (**raw**), MIF in accordance to [src_mif(5)][] (**mif**) or none (**debug**), in which case it will be printed to [stderr(3)][]. The MIF format is used by default. Output files are named after and written to the same directory as input *file*s.
